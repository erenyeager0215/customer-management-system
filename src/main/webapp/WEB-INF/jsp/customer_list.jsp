...
<body>
	...
	<table class="table table-striped">
		...
		<tbody>
			 ...
			 <tr>
			 	<td><a href="<%=request.getContextPath() %>/CustomerDetailServlet?id=<%= cus.getId() %>"><%= cus.getId() %></a></td>	
			 	<td><%= cus.getName() %></td>
			 	<td><%= cus.getAddress() %></td>
			 	<td><%= cus.getTel_number() %></td>
			 	<td><form  action="<%=request.getContextPath() %>/CustomerDeleteServlet" method="GET">
			 	<button name="id" value="<%=cus.getId() %>" onclick="return confirm('本当に削除してもよろしいですか？')">削除</button>
			 	</form></td>
			 </tr>
			 <% } %>
		</tbody>
